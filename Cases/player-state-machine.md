...
- 플레이어의 상태에 따라 **다른 키보드 입력**을 받을 수 있어야 합니다.
...
부르기 때문에, 어떤 상태 enum이 어떤 상태 클래스에 대응하는지가 이 함수 안에서만
결정됩니다. 상태에 따라 다른 객체를 물려야 하는 경우도 여기서 처리했습니다 —
예를 들어 `IDLE`은 3D이면서 사이버 모드일 때만 `CyberIdle`로 갈라집니다.
...
- **장점 — 상태별로 입력 해석이 분리됩니다.** 같은 스페이스바가 서 있을 때는 점프,
  ...
  보게 했습니다.
- **장점 — 애니메이션과 맞물리는 지점이 정해져 있습니다.** 모션이 끝나야 풀리는
  동작은 `On_AnimEnd()`만 채우면 되고, 상태마다 종료 타이밍을 따로 재지 않습니다.
- **장점 — 구조를 모르는 사람도 상태를 추가할 수 있었습니다.** 개발 막바지에
...
- **단점 — 상태를 나누는 기준이 없었습니다.** 복합적인 상태를 어떻게 정의할지
  기준을 정해두지 않아서, '점프 공격'을 *"JUMP 상태 중 공격 입력을 받으면 공격"*
  으로 볼지 *"ATTACK 상태에서 점프 중인지에 따라 분기"* 로 볼지가 그때그때
...
성능이 문제가 된 구조가 아니어서 **수치는 측정하지 않았습니다.** 대신 개발 중에
같은 문제를 다르게 풀 수 있는 방법을 두 갈래로 정리해뒀습니다.
...
수 없을까 싶어 `CTransition`·`Condition`을 옮겨 오기까지 했지만, 붙이지는
못했습니다. 두 클래스는 지금도 프로젝트에 남아 있고, 어디에서도 생성되지 않습니다.

### 상태를 평면으로 둘 것인가, 계층으로 나눌 것인가

| | 평면 — 선택 | MainState + SubState |
|---|---|---|
...
공격'처럼 두 축이 겹치는 동작이 나올 때마다 조합 하나가 새 클래스 하나가 됩니다.

**다만 이건 처음부터 저울질한 대안은 아닙니다.** 설계 시점에는 계층 구조를 후보로
놓고 검토하지 않았고, 상태 클래스가 계속 늘어난 뒤에야 필요를 느꼈습니다.
...
4번은 사실 새로 떠올린 방법이 아닙니다. **직전 프로젝트의 상태 머신이 이미 메인
