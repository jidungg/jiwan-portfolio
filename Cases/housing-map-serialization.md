...
### 어려움 2 - 아이템 489종 자동 등록: 모델 파일을 넣는 만큼 늘어나는 구조

건설 아이템 데이터는 json에 손으로 적지 않습니다. `CItemDataBase::Load_Data()`가
...
남은 문제는 아이콘입니다. 489종을 손으로 그릴 수도 없었고, 원작에서 아이콘
리소스를 구하지도 못했습니다. 그래서 **로딩 때 모델을 오프스크린으로 한 번씩
찍어 아이콘을 만듭니다.** 300×300 렌더 타겟 하나를 잡아두고 뷰·투영 행렬을
...
- **장점** — 파일을 직접 열어 손으로 고치는 게 빠릅니다. 같은 블록을 길게
  ...
  셀 좌표가 정수 인덱스 하나로 접혀 있어서 포맷이 단순해지는 것도 이득입니다.
  좌표 세 개 대신 `Index` 하나만 적으면 되고, "연속"이라는 개념도 인덱스 +1로
  바로 판정됩니다.
...
- **단점** — **압축이 X축 방향에만 걸립니다.** 인덱스가 `x + X*z + X*Z*y`라
  ...
  **`CTerrainObject::ToJson()`이 `IntData`/`FloatData`를 항상 빈 배열로
  내보냅니다.** 몬스터 스포너나 포탈처럼 블록 타입별 추가 데이터를 쓰는
  오브젝트는 저장할 때 그 데이터가 빠집니다. 의도한 동작이 아니라 놓친
  부분입니다.
...
**바이너리 직렬화**를 검토했다가 접었습니다. 파일을 직접 열어서 수정하는 게 빠르고
