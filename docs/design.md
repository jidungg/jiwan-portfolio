...
STAR 대신 다음 구성을 쓴다. 앞의 네 개가 본체이고, '회고'는 쓸 말이 있을 때만 붙이는 선택 섹션이다.
...
| 회고 *(선택)* | 지금 다시 하면 무엇을 다르게 할까 | 성장 가능성 신호. 쓸 말이 없으면 뺀다 |
...
최종 PDF에서 사례와 섹션에 번호를 붙인다. 사례 파일에는 번호를 적지 않는다 — 같은 사례가 JD마다 다른 순서로 조립되므로, 번호는 `build.py`가 manifest 순서대로 부여한다.

- 목차: `1.` `2.` … + 사례 제목
- 사례 제목: `1. 그리드 큐브맵 Raycast에 3D DDA 도입` (h2)
- 사례 내 섹션: `1.1 배경`, `1.2 구현`, `1.3 장단점` … (h3)

본문의 `## 섹션`은 렌더링 시 h3로 낮아진다. 사례 제목이 h2를 차지해 계층이 맞아야 하기 때문이다.
...
- 사례·섹션 번호 부여 (`1.`, `1.1`), 목차 생성
- `![alt](src)`를 `<figure>` + `<figcaption>`으로 감싸 캡션을 노출
