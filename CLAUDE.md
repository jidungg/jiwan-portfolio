...
2. **소스에 없는 수치·기술을 지어내지 않는다.** 확인 안 되면 사용자에게 묻는다.
   ...
   특히 **사실과 판단을 구분한다.**
   - *코드·문서에서 확인되는 사실* (무엇을 하는가, 몇 μs인가, 어디서 호출되는가)
     → 확인 후 쓴다.
   - *작성자의 판단·의도·감정* (왜 그렇게 짰는가, 무엇이 아쉬운가, 왜 안 고쳤는가,
     지금 다시 하면 어떻게 할 것인가) → **사용자가 말한 것만 쓴다.**
     기술적으로 그럴듯해도, 추론이 맞을 것 같아도 쓰지 않는다.

   회고 섹션이 특히 위험하다. 1인칭 서술이라 지어내면 **사용자가 하지도 않은
   생각을 본인 입으로 말한 것처럼** 된다. 면접에서 그 문장으로 질문이 들어온다.
   채울 내용이 없으면 `확인 필요: <무엇을 물어야 하는지>`로 남기고 묻는다.
...
6. **결과물이 보기 좋은지는 사용자가 판단한다.** PDF를 이미지로 변환해 들여다보지
   않는다. 렌더링 후에는 PDF 경로만 알려주고, 사용자가 보고 말해주는 것을 고친다.
   "정상 렌더링됐다" 같은 표현도 쓰지 않는다 — 스크립트가 오류 없이 끝났으면
   그렇게만 말한다.
...
본문은 **배경 → 구현 → 장단점 → 대안 비교** 순으로 쓰고, 쓸 말이 있으면 끝에
**회고**를 붙인다. 회고는 선택이다 — 없으면 넣지 않는다.
